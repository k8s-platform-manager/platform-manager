...
```mermaid
flowchart TB
    PRJ[Project]:::app --> A[Application]:::app --> AI[ApplicationInstance]:::app
...
| **EnvironmentType** | Aggregate (vocabulary entry) | Platform team (defaults shipped by controller) | A category of environment used as the stable hook for policy scoping, cross-environment rules, and shared settings. The controller ships four standard defaults — `ephemeral`, `development`, `staging`, `production` — and the vocabulary may extend the set. `ephemeral` additionally implies lifecycle semantics (TTL / auto-teardown) that the controller mechanism must support. Each entry declares any **attribute key/values** instances of that type carry into resolution (e.g. `tier: high-availability`) — this is where environment-flavored settings live; there is no separate per-environment object to configure. It also declares **`defaultReferenceTarget`** — which EnvironmentType its references resolve to, defaulting to *self*; the shipped `ephemeral` entry targets `development` (R8, R12, RFC-003) — and **`teardownSemantics`** (`blocking` \| `eager`, see R6; shipped `eager` for `ephemeral`, `blocking` otherwise). |
| **ApplicationInstance** | Aggregate | App team (created by the act of deploying) | The materialization of an Application against an **EnvironmentType**, created by an explicit deploy action (e.g. `platform-manager deploy --app app.yaml --environment production`), not by first authoring a separate "environment" object. Carries the EnvironmentType and an optional **name**, needed only to disambiguate multiple concurrent instances of the same type (e.g. several `ephemeral` PR instances: `pr-123`, `pr-456`). ApplicationInstances — not Applications — are the owners of Resources; the same SCORE declaration materializes a **separate** Resource per instance. |
| **Deployment** | Aggregate | App team (triggered); Controller (status) | An append-only record of applying a specific Application revision to an ApplicationInstance. Carries a status (`in-progress`, `succeeded`, `failed`). The current state of an ApplicationInstance is defined by its **latest successful Deployment**; Resources belong to the ApplicationInstance, never to a Deployment. Rollback is expressed as a *new* Deployment of a previously good revision (see R11). The *first* Deployment for a given (Application, EnvironmentType, name) tuple is what brings the ApplicationInstance into existence. |
...
| **ResolutionContext** | Value object | Controller (computed) | The input tuple assembled per resolution: EnvironmentType (including its declared attributes) + application/instance **identity** (names — never metadata, see C6, RFC-002) + the Resource's type, class, params, and requested accesses. Assembled directly by the Resolver from these fixed inputs — there is no side channel; Policies are applied *over* the context by the Resolver, never contributed *into* it. Not stored as an object, but its schema is documented and every resolution trace serializes the exact context used. |
...
A Resource with active consumer bindings **blocks deletion** (finalizer semantics) rather than cascading. The one exception is vocabulary-declared, not hardcoded: if the *owning* instance's EnvironmentType declares `teardownSemantics: eager` (the shipped default for `ephemeral`), TTL/teardown deletion proceeds — surviving consumer bindings are force-revoked, and the revocation is recorded explainably on both the deleted resource's trace and each affected consumer, so the audit trail survives even when the resource does not. The blockage is surfaced through the explainability API. Because every access grant is an RBR→ResourceBinding pair, "what breaks if I delete this Resource" is a graph query over its bindings — the same data the explainability surface already holds, rendered as a dependency ("blast radius") view. A UI showcasing all affected systems is an explicit roadmap item built on this query.
...
A **Deployment** is the append-only record of applying one Application revision to one ApplicationInstance; the ApplicationInstance's current state is always its **latest successful Deployment**. This yields rollback for free — with three deliberate semantics:
...
**C4 — Region is context, not environment.** An ApplicationInstance deliberately carries no infrastructure attributes beyond its EnvironmentType. A production instance may span regions; how region-like attributes enter the ResolutionContext is *not yet modeled* (see §9). Putting region directly on the instance was considered and rejected as smuggling infrastructure back into the intent layer.

**C5 — EnvironmentType defaults are conventions, not mechanism.** The controller ships `ephemeral` / `development` / `staging` / `production` as standard defaults to keep policies portable across organizations, but the set is vocabulary-extensible. Hardcoding the enum would violate principle 1. The one mechanical commitment is that `ephemeral` lifecycle semantics (TTL, auto-teardown) are supported by the controller. **Current proposal**: an `ephemeral` ApplicationInstance spins up on PR open against the source repository and tears down on PR close or merge — details to be defined (see Q6), and note this implies a source-control event source (§9). `ephemeral`'s reference-target and teardown behaviors are likewise vocabulary properties on the EnvironmentType entry, not hardcoded mechanism (R6; R8/R12, RFC-003).
...
| Organization-level Environment objects | The environment dimension is application-scoped (now via ApplicationInstance, see below). Org-level environments forced unrelated applications to share configuration objects and invited infrastructure attributes into intent. |
| Region (or other infrastructure attributes) on the environment dimension | See C4. |
| **Environment as its own aggregate/CRD, separate from ApplicationInstance** | Rejected 2026-08-07 (reversing an earlier draft). App teams should configure nothing beyond application resources — not a standalone environment object. Settings that would have lived on Environment now live on EnvironmentType (shared, platform-owned); the only per-environment fact an app team supplies — a name, needed solely to disambiguate multiple instances of the same EnvironmentType (e.g. concurrent `ephemeral` PRs) — moved onto ApplicationInstance itself, created by the deploy action rather than authored ahead of time. |
...
| Generic **AttributeSource** registration/extension-point layer for the ResolutionContext (v1) | Rejected 2026-08-07. The ResolutionContext has exactly one producer per input today (EnvironmentType attributes, identity, resource declaration), so a registration/provenance abstraction had no second producer to justify it. Revisit only if/when the deployment-target input (§9) is actually scoped and a real second producer exists. |
...
**Identity (teams, principals, ownership of Projects).** Referenced by three settled mechanisms that currently point at a hole: RBR approval ("owners of the workload"), instantiation rights ("who may deploy Application X as a `production`-type instance"), and Project ownership. Decision: these will be folded into **one** identity model rather than several ad-hoc ones — the subject of **RFC-004**. Project is the designated anchor (teams own Projects; Applications inherit), now strengthened by Project's role as the naming scope (R2, RFC-002).

**Runtime / deployment target.** The source of infrastructure-flavored context attributes (region, cluster, cloud). Exactly how it will plug into the ResolutionContext is not yet designed. Humanitec's equivalent concept confirms the slot exists.
...
|---|---|---|
| Q1 | Who may instantiate: which principals can deploy an Application as an instance of a given EnvironmentType? | Deferred to the identity model (§9 / RFC-004); enforced via Policy over (team × EnvironmentType). |
| Q2 | How do runtime/deployment-target attributes (region, cluster, cloud) enter the ResolutionContext? | Open again as of 2026-08-07 — the generic AttributeSource extension point that was going to answer this was retired (see decision log, §8) for lacking a real second producer. Deferred until the deployment-target concept (§9) is actually scoped. |
