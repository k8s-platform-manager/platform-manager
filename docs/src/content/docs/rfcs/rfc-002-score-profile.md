...
| ApplicationInstance | (Application + Environment, combined) | — | The *term* is ours; the *split* it encodes (env-independent definition vs env-scoped materialization) matches Humanitec's Application/Environment structure. Unlike Humanitec, there is no separate Environment object to author first — an ApplicationInstance is created directly by deploying, carrying an EnvironmentType and an optional disambiguating name. |
| Deployment | Deployment | — | Aligned concept and name: an event with status applying a revision to an ApplicationInstance. |
| EnvironmentType | Environment (type/category aspect only) | — | Humanitec's Environment is one object; we split it into EnvironmentType (shared, platform-owned settings) plus the per-instance name carried on ApplicationInstance (see rejected ideas, RFC-001 §8). |
